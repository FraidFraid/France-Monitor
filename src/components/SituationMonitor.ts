...
import type { DetectedSituation, SituationAction } from '../types/index.ts';
...
export class SituationMonitor {
  ...
  private readonly handlePointerMove = (event: PointerEvent): void => {
    ...
    if (!this.dragging && Math.hypot(deltaX, deltaY) < SituationMonitor.DRAG_THRESHOLD_PX) {
      return;
    }
  ...
  private persistPosition(): void {
    localStorage.setItem(SituationMonitor.STORAGE_KEY, JSON.stringify({
  ...
  private restorePosition(): void {
    const raw = localStorage.getItem(SituationMonitor.STORAGE_KEY);
    if (!raw) {
    ...
    try {
      ...
      localStorage.removeItem(SituationMonitor.STORAGE_KEY);
    }
