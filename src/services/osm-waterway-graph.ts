...
export type OsmWaterwayFeature = Feature<LineString, OsmWaterwayProperties>;

export interface OsmSegmentEdge {
  fromNodeKey: string;
  toNodeKey: string;
...
export interface OsmProjectedNode {
  node: OsmGraphNode;
  distanceKm: number;
  wayId: string | number;
}
...
export function projectToNearestGraphNode(
  target: Coordinate,
  features: OsmWaterwayFeature[],
): OsmProjectedNode | null {
  let best: OsmProjectedNode | null = null;
...
export function reconstructOsmLineString(path: OsmDirectedEdge[]): Coordinate[] {
