...
describe('cyber-threat-scoring · computeCyberPressureAssessment', () => {
  it('retourne un score nul sans aucun signal', () => {
    const noSignal = computeCyberPressureAssessment(null, []);
    assert.equal(noSignal.score, 0);
  });

  it('rend visible mais borne un score ransomware seul', () => {
  ...
  it('borne un score exposition seule sous le ransomware seul', () => {
    const ransomwareOnly = computeCyberPressureAssessment(null, [
      makeThreatEvent('ran-1', 'ransomware', 'high', daysAgo(1)),
      makeThreatEvent('ran-2', 'ransomware', 'critical', daysAgo(5), { sector: 'Santé' }),
    ]);
  ...
  it('agrège la pression combinée avec bonus de corrélation', () => {
  ...
  it('atténue les signaux périmés (fenêtre de fraîcheur)', () => {
  ...
  it('conserve le fallback legacy quand aucun évènement récent', () => {
