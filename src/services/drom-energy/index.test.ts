...
describe('drom-energy · dashboard', () => {
  it('crée un dashboard vide avec les 5 territoires', () => {
    const empty = createEmptyDromEnergyDashboard();
    assert.equal(empty.assets.length, 0);
    assert.equal(empty.territories.length, 5);
  });

  it('normalise une feature poste source de La Réunion', () => {
  ...
  it('construit le contenu des tooltips (complet et partiel)', () => {
  ...
  it('construit le dashboard postes sources à partir d’une FeatureCollection', () => {
  ...
  it('charge le dashboard statique via une implémentation fetch mockée', async () => {
    const empty = createEmptyDromEnergyDashboard();
    const datasetMeta = getDromEnergySources().find((source) => source.id === 'postes_sources_reunion');
    assert.ok(datasetMeta);
