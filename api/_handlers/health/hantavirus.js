...
export default async function handler(req, res) {
  ...
  // Les 3 sources amont sont indépendantes réseau (même si PEPPS n'est *utilisé*
  // que si DGS n'a rien trouvé) : on les lance en parallèle plutôt qu'en série
  // (jusqu'à ~30s cumulés à 10s de timeout chacune avant ce correctif) et on
  // rejoue ensuite la même logique conditionnelle sur les résultats.
  const [dgsResult, peppsResult, spfResult] = await Promise.allSettled([
    fetchText(DGS_URGENT_INDEX_URL, { timeoutMs: 10000 }),
    fetchText(PEPPS_HANTAVIRUS_INDEX_URL, { timeoutMs: 10000 }),
    fetchText(SPF_HANTAVIRUS_DATA_URL, { timeoutMs: 10000 }),
  ]);

  if (dgsResult.status === 'fulfilled') {
    const html = dgsResult.value;
    ...
    console.warn('[api/health/hantavirus] DGS scan failed:', dgsResult.reason instanceof Error ? dgsResult.reason.message : String(dgsResult.reason));
  }

  if (dgsCandidatePdfs.length === 0) {
    if (peppsResult.status === 'fulfilled') {
      const peppsHtml = peppsResult.value;
      ...
      console.warn('[api/health/hantavirus] PEPPS scan failed:', peppsResult.reason instanceof Error ? peppsResult.reason.message : String(peppsResult.reason));
    }
  }

  if (spfResult.status === 'fulfilled') {
    const spfHtml = spfResult.value;
    ...
    console.warn('[api/health/hantavirus] SPF scan failed:', spfResult.reason instanceof Error ? spfResult.reason.message : String(spfResult.reason));
  }
