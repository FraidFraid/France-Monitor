 ...
 *
 * Cache : swr-cache (mémoire + Redis) — 15 min frais, tolérance 1 h en cas de lenteur/panne ENTSOG.
 */
...
const CACHE_KEY = 'swr:energy:gas-pir';
...
async function produceGasPir() {
  ...
  return { points, fetchedAt: new Date().toISOString(), status };
}
...
export default async function handler(req, res) {
  ...
  try {
    const { value: data, cache } = await getOrRefresh(
      CACHE_KEY,
      { ttlSec: CACHE_TTL_SEC, staleSec: CACHE_STALE_SEC, timeoutMs: 12_000 },
      produceGasPir,
    );
