/**
 * api/threats.js — Vercel Edge Function
 * Endpoint dédié aux menaces cyber cartographiées.
 *
 * Agrège FrenchBreaches + HIBP France + RansomwareLive + CERT-FR et normalise en ThreatEvent[].
 * Cache interne 10 minutes (Edge cache Vercel + Cache-Control).
 *
 * Sécurité :
 *   - Pas d'IPs exposées ni données personnelles
 *   - Sources publiques légales uniquement
 *   - Rate limiting implicite via Edge cache Vercel
 *   - Clé API optionnelle pour les sources premium (SHODAN, CENSYS — Phase 4)
 *
 * GET /api/threats
...
const FRANCE_META = { code: 'FR', name: 'France', flag: '🇫🇷', coordinates: [2.2137, 46.2276] };
...
const SCRAPLING_PROXY_URL = process.env.SCRAPLING_PROXY_URL || process.env.VITE_SCRAPLING_PROXY_URL || '';
const companyGeoCache = new Map();
const KNOWN_COMPANY_LOCATIONS = {
  legilog: {
...
function absoluteFrenchBreachesUrl(href) {
  return new URL(decodeHtml(href), FRENCH_BREACHES_BASE_URL).toString();
}

function parseFrenchBreachesDate(raw) {
...
async function fetchFrenchBreachesHtml(url) {
  const headers = {
...
async function fetchFrenchBreachesJson(url) {
...
function isInformativeRansomwareDescription(value) {
  const text = stripHtml(value);
  return text && !/^\[?ai generated\]?\s*n\/?a$/i.test(text) && !/^n\/?a$/i.test(text);
}

function formatDateFr(value) {
  const date = new Date(value);
  return Number.isFinite(date.getTime()) ? date.toLocaleDateString('fr-FR') : null;
}

function formatEmployeeRange(code) {
  const ranges = {
    '00': '0 salarié',
    '01': '1 à 2 salariés',
    '02': '3 à 5 salariés',
  ...
  return ranges[String(code || '')] || undefined;
}

function buildCompanyProfile(result) {
  ...
  return {
    ...
    createdAt: formatDateFr(result.date_creation) || undefined,
    openEstablishments: Number.isFinite(Number(result.nombre_etablissements_ouverts))
      ? Number(result.nombre_etablissements_ouverts)
      : undefined,
  };
}

function buildRansomwareSummary(victim, geo, domain, organizationProfile) {
...
function isFranceBreach(breach) {
...
function parseApproxRecords(text) {
  const match = /(\d+(?:[\s.,]\d+)*)\s*(milliards?|millions?|k|records?|lignes?|clients?|victimes?|personnes?|comptes?|adhérents?|licenciés?)/i.exec(String(text || ''));
...
function extractArrayPayload(payload) {
  ...
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) return candidate.filter((item) => item && typeof item === 'object' && !Array.isArray(item));
  }
  if (Array.isArray(payload.features)) {
    return payload.features
      .map((feature) => ({ ...(feature.properties || {}), geometry: feature.geometry }))
...
function normalizeFrenchBreachesMapEvent(raw, index) {
...
async function fetchFrenchBreachesMapEvents() {
...
function extractKnownDataClasses(text) {
  const known = [
    'Adresse email',
    'Email',
    'Nom et prénom',
...
function parseFrenchBreachesArchive(html) {
  const alertBlock = String(html || '').split(/Articles de Blog/i)[0] || String(html || '');
  const entries = [];
  const seen = new Set();
...
function parseFrenchBreachesDetail(html) {
...
function shouldEnrichFrenchBreachesDetail(index) {
  return index < Math.max(0, FRENCH_BREACHES_DETAIL_LIMIT);
}

async function resolveFrenchBreachesGeo(entry, index) {
  ...
  return {
    label: 'France',
    coordinates: [FRANCE_META.coordinates[0] + offset[0], FRANCE_META.coordinates[1] + offset[1]],
    precision: 'country',
    source: 'FrenchBreachesAdapter country fallback',
  };
}

function normalizeFrenchBreachesEvent(entry, detail, location) {
  const key = entry.title.toLowerCase().replace(/[^a-z0-9]/g, '');
  ...
  return {
    ...
    countryCode: FRANCE_META.code,
    countryName: FRANCE_META.name,
    flagEmoji: FRANCE_META.flag,
...
async function fetchFrenchBreachesLeaks() {
...
function resolveHibpLocation(breach, index) {
  ...
  return {
    label: FRANCE_META.name,
    coordinates: [
      FRANCE_META.coordinates[0] + offset[0],
      FRANCE_META.coordinates[1] + offset[1],
    ],
    precision: 'country',
  };
}
...
async function fetchHibpBreaches() {
  const resp = await fetch('https://haveibeenpwned.com/api/v3/breaches', {
    headers: {
      'User-Agent': 'FranceMonitor/1.0 breach-map',
  ...
  return breaches
    ...
    .sort((a, b) => new Date(b.AddedDate || b.ModifiedDate || b.BreachDate).getTime() - new Date(a.AddedDate || a.ModifiedDate || a.BreachDate).getTime())
    .slice(0, 304)
    .map((b, index) => {
...
/** Résolution géo minimale basée sur région/pays connu */
function resolveGeo(victim) {
  const city = victim.city;
  const region = victim.region;

  // Quelques métropoles françaises hardcodées pour la précision
  const FRENCH_CITIES = {
  ...
  if (city) {
    ...
    if (FRENCH_CITIES[key]) {
      return { coordinates: FRENCH_CITIES[key], label: city, precision: 'city' };
    }
  }

  // Centroïde France par défaut
  return { coordinates: [2.2137, 46.2276], label: region || 'France', precision: region ? 'region' : 'country' };
}

function normalizeCompanyQuery(input) {
...
function extractDomainName(website) {
  if (!website) return '';
  try {
    const host = new URL(String(website).startsWith('http') ? website : `https://${website}`).hostname;
...
function buildCompanyQueries(victim) {
  const candidates = [
    victim.post_title,
    victim.company,
    victim.organization,
...
function parseSireneCoordinates(siege) {
...
function scoreCompanyResult(result, query) {
  const haystack = [
    result?.nom_complet,
...
async function resolveCompanyGeo(victim) {
  const queries = buildCompanyQueries(victim);
  for (const query of queries) {
    const known = KNOWN_COMPANY_LOCATIONS[query.replace(/\s+/g, '')];
    if (known) return known;
    if (companyGeoCache.has(query)) return companyGeoCache.get(query);

    try {
      const url = `${ENTREPRISE_SEARCH_URL}?q=${encodeURIComponent(query)}&per_page=3`;
      const resp = await fetch(url, {
        headers: { 'User-Agent': 'FranceMonitor/1.0 company-geocoder', Accept: 'application/json' },
      ...
      const match = results
        ...
        .filter((item) => item.coordinates && item.score >= 0.5)
      ...
      if (match) {
        ...
        const geo = {
          ...
          label: siege.libelle_commune || match.result.nom_raison_sociale || victim.city || 'France',
          precision: 'hq',
          address: siege.adresse || undefined,
      ...
      // Enrichissement best-effort: l'événement reste affiché avec le fallback géographique.
    }
  }
  return null;
}
...
// ─── Source 1 : RansomwareLive ───

async function fetchRansomwareLive() {
  const url = 'https://data.ransomware.live/posts.json';
  const resp = await fetch(url, {
    headers: {
      'User-Agent': 'FranceMonitor/1.0 (https://francemonitor.fr)',
      'Accept': 'application/json',
    },
    signal: AbortSignal.timeout(12_000),
  });
  ...
  return mapLimit(victims, 4, async (v, i) => {
      const geo = await resolveCompanyGeo(v) || resolveGeo(v);
      let domain;
      if (v.website) {
        try {
      ...
      return {
        id: `ransomware-live-${i}-${new Date(v.discovered).getTime()}`,
        type: 'ransomware',
        ...
        organizationProfile,
        date: v.discovered,
        summary: buildRansomwareSummary(v, geo, domain, organizationProfile),
...
// ─── Source 2 : CERT-FR RSS ───

async function fetchCertFrAlerts() {
  const certUrl = 'https://www.cert.ssi.gouv.fr/feed/';
  // Le RSS proxy interne n'est pas disponible depuis une Edge Function Vercel,
  // on fetch le RSS directement (le CORS n'est pas un problème server-side).
  const resp = await fetch(certUrl, {
    headers: {
      'User-Agent': 'FranceMonitor/1.0 Threat Aggregator',
  ...
  // Parser RSS minimal (pas de lib disponible en Edge runtime)
  const items = [];
  ...
  while ((match = itemRegex.exec(xml)) !== null && items.length < 15) {
    ...
    if (!title || !pubDate) { i++; continue; }

    const pub = new Date(pubDate);
    if (pub.getTime() < thirtyDaysAgo) { i++; continue; }

    items.push({
      id: `cert-fr-${i}-${pub.getTime()}`,
      type: 'vulnerability',
...
export default async function handler(request) {
  ...
  try {
    const [frenchBreachesMapResult, frenchBreachesResult, hibpResult, ransomwareResult, certFrResult] = await Promise.allSettled([
    ...
    const frenchBreachesMap = frenchBreachesMapResult.status === 'fulfilled' ? frenchBreachesMapResult.value : [];
    const frenchBreaches = frenchBreachesResult.status === 'fulfilled' ? frenchBreachesResult.value : [];
    const hibp = hibpResult.status === 'fulfilled' ? hibpResult.value : [];
    const ransomware = ransomwareResult.status === 'fulfilled' ? ransomwareResult.value : [];
    const certFr = certFrResult.status === 'fulfilled' ? certFrResult.value : [];

    // Déduplication par (organizationName + type)
    const seen = new Set();
