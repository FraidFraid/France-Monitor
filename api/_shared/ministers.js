import { GOUVERNEMENT_DATA } from '../../src/config/government-data.js';

const GOUVERNEMENT = GOUVERNEMENT_DATA;
const SCRAPLING_PROXY_URL = process.env['VITE_SCRAPLING_PROXY_URL'] || process.env['SCRAPLING_PROXY_URL'] || 'http://localhost:8080';
const OFFICIAL_SOURCE_STATUS = {
  verificationStatus: 'official-live',
  sourceKind: 'official',
  identityConfidence: 'high',
};
const DIRECTORY_SOURCE_STATUS = {
...
export function applyMinistersCors(res) {
...
function slugifyPerson(prenom, nom) {
...
async function fetchViaScrapling(targetUrl) {
...
async function fetchPageHtml(url, timeoutMs = 6000) {
  ...
  try {
    ...
    if (!response.ok) return await fetchViaScrapling(url);
    const html = await response.text();
    if (!html || isBlockedHtml(html)) return await fetchViaScrapling(url);
    return html;
  } catch {
...
function extractProfilePhoto(html, pageUrl) {
  ...
  if (srcSetMatch?.[1]) {
    ...
    if (firstCandidate && !firstCandidate.startsWith('data:')) return absolutizeUrl(firstCandidate, pageOrigin);
  }
...
function extractOfficialBio(html) {
...
function buildProfileCandidates(minister) {
  const personSlug = slugifyPerson(minister.prenom, minister.nom);
  const candidates = [];
  if (minister.isPresident) candidates.push({ url: 'https://www.elysee.fr/emmanuel-macron', label: 'Élysée' });
  ...
  if (minister.sourceUrl) candidates.push({ url: minister.sourceUrl, label: 'Source officielle' });
  return candidates.filter((candidate, index, array) => array.findIndex((item) => item.url === candidate.url) === index);
}

async function fetchOfficialProfileMeta(minister) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      return {
        ...
        sourceChain: [{ label: candidate.label, url: candidate.url, kind: 'official' }],
        ...DIRECTORY_SOURCE_STATUS,
      };
    } catch {}
  }
  ...
  return {
    ...
    sourceChain: fallback.url ? [{ label: fallback.label, url: fallback.url, kind: 'official' }] : [],
...
function extractAgendaLink(html) {
...
function extractDownloadLink(html, extension, pageUrl) {
...
function normalizeAgendaText(value) {
  return decodeHtmlEntities(stripTags(value)).replace(/\u00a0/g, ' ').replace(/\s+/g, ' ').trim();
}
...
function formatDateDisplay(dateValue, timeValue) {
  const rawDate = (dateValue ?? '').trim();
  const rawTime = (timeValue ?? '').trim();
...
function parseAgendaCsv(csv, sourceUrl, sourceLabel) {
  ...
  const headerIndex = (patterns) => header.findIndex((value) => patterns.some((pattern) => pattern.test(value)));
...
async function fetchAgendaPageItems(agendaUrl) {
  const agendaHtml = await fetchPageHtml(agendaUrl);
  if (!agendaHtml) {
    return [{ title: 'Consulter l\'agenda officiel', date: 'Agenda officiel', sourceLabel: new URL(agendaUrl).hostname, url: agendaUrl }];
  }
  ...
  if (csvUrl) {
    try {
      const csvResponse = await fetch(csvUrl, { headers: { Accept: 'text/csv,text/plain,*/*' }, signal: typeof AbortSignal.timeout === 'function' ? AbortSignal.timeout(10000) : undefined });
      if (csvResponse.ok) {
    ...
    return [{ title: 'Télécharger l\'agenda officiel', date: 'Agenda officiel', sourceLabel: 'info.gouv.fr (CSV)', url: csvUrl }];
  }
  const pdfUrl = extractDownloadLink(agendaHtml, 'pdf', agendaUrl);
  if (pdfUrl) return [{ title: 'Télécharger l\'agenda officiel', date: 'Agenda officiel', sourceLabel: 'info.gouv.fr (PDF)', url: pdfUrl }];
  return [{ title: 'Consulter l\'agenda officiel', date: 'Agenda officiel', sourceLabel: new URL(agendaUrl).hostname, url: agendaUrl }];
}

function extractAgendaSectionHtml(html) {
...
function parseAgendaSectionFromHtml(html, sourceUrl, sourceLabel) {
  const sectionHtml = extractAgendaSectionHtml(html);
  if (!sectionHtml) return [];
  const paragraphs = Array.from(sectionHtml.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)).map((match) => normalizeAgendaText(match[1] ?? '')).filter(Boolean);
  if (paragraphs.length > 0) {
    ...
    for (let i = 0; i < paragraphs.length; i += 1) {
      ...
      items.push({ title: line, date: currentDate || 'Agenda officiel', location, sourceLabel, url: sourceUrl });
    }
...
function parseRfc2822DateFR(raw) {
  try {
    const d = new Date(raw);
    if (Number.isNaN(d.getTime())) return null;
    return { iso: d.toISOString(), display: d.toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' }) };
  } catch {
    return null;
  }
}

function sortAgendaByDateDesc(items) {
...
async function fetchElyseeAgenda() {
  try {
    const signal = typeof AbortSignal.timeout === 'function' ? AbortSignal.timeout(5000) : undefined;
    const r = await fetch('https://www.elysee.fr/rss/actualites.xml', { headers: { Accept: 'application/rss+xml, text/xml', 'User-Agent': 'FranceMonitor/2.0' }, signal });
    if (r.ok) {
      ...
      if (!isBlockedHtml(xml) && xml.includes('<item>')) {
        ...
        if (rssItems.length > 0) return sortAgendaByDateDesc(rssItems).slice(0, 8).map(({ _isoDate, ...rest }) => rest);
      }
    }
  } catch {}
  ...
  if (html) {
    ...
    while ((match = articleRegex.exec(html)) !== null && items.length < 10) {
      ...
      items.push({ title, date: dateDisplay || isoDate || 'Élysée', _isoDate: isoDate || undefined, sourceLabel: 'Élysée', url: href.startsWith('http') ? href : `https://www.elysee.fr${href}` });
    }
    if (items.length > 0) return sortAgendaByDateDesc(items).slice(0, 8).map(({ _isoDate, ...rest }) => rest);
  }
  return [{ title: 'Actualités et agenda du Président de la République', date: 'Agenda officiel', sourceLabel: 'Élysée', url: 'https://www.elysee.fr/toutes-les-actualites' }];
}

async function fetchOfficialAgenda(minister) {
  if (minister.isPresident) return fetchElyseeAgenda();
  const fallbackMinister = GOUVERNEMENT.find((entry) => entry.prenom === minister.prenom && entry.nom === minister.nom);
  const knownAgendaUrl = minister.agendaUrl ?? fallbackMinister?.agendaUrl;
  if (knownAgendaUrl) {
    try {
      if (knownAgendaUrl.includes('/agenda/ministre/')) return await fetchAgendaPageItems(knownAgendaUrl);
      ...
      if (profileHtml) {
        ...
        if (linkedAgendaUrl) return await fetchAgendaPageItems(linkedAgendaUrl);
      }
    } catch {}
    return [{ title: `Consulter l'agenda officiel de ${minister.prenom} ${minister.nom}`, date: 'Agenda officiel', sourceLabel: new URL(knownAgendaUrl).hostname, url: knownAgendaUrl }];
  }
  const candidates = buildProfileCandidates({ ...minister, siteMinistere: fallbackMinister?.siteMinistere, sourceUrl: fallbackMinister?.sourceUrl });
  for (const candidate of candidates) {
    try {
      ...
      if (agendaUrl.includes('/agenda/ministre/')) return await fetchAgendaPageItems(agendaUrl);
      const agendaHtml = await fetchPageHtml(agendaUrl);
      if (!agendaHtml) return [{ title: `Consulter l'agenda officiel de ${minister.prenom} ${minister.nom}`, date: 'Agenda officiel', sourceLabel: candidate.label, url: agendaUrl }];
      ...
      return [{ title: `Consulter l'agenda officiel de ${minister.prenom} ${minister.nom}`, date: 'Agenda officiel', sourceLabel: candidate.label, url: agendaUrl }];
    } catch {}
  }
  ...
  return [{ title: `Consulter l'actualité et l'agenda de ${minister.prenom} ${minister.nom}`, date: 'Agenda officiel', sourceLabel: fallback.label, url: fallback.url }];
}

function normalizeMinisterFromLine(line) {
  ...
  if (lastNameParts.length === 0) lastNameParts.unshift(nameParts.pop() ?? '');
  const prenom = toTitleCaseName(nameParts.join(' '));
  const nom = toTitleCaseName(lastNameParts.join(' '));
  const ministerId = title.toLowerCase().includes('premier ministre') ? 'pm' : slugifyId(title);
  return { id: ministerId, prenom, nom, titre: title.charAt(0).toUpperCase() + title.slice(1), titreShort: inferShortTitle(title), isPM: title.toLowerCase().includes('premier ministre') };
}

async function fetchPrimeMinisterRecord() {
...
async function fetchGovernmentCompositionFromJORF() {
  const record = await fetchPrimeMinisterRecord();
  if (!record) return [];
  const references = tryParseArray(record.texte_reference);
  const compositionUrl = references.find((item) => item.libelle?.toLowerCase().includes('composition du gouvernement'))?.valeur;
  ...
  const text = stripTags(articleMatch[1]).replace(/\s*;\s*/g, ';\n').replace(/\s*Mme\s/g, '\nMme ').replace(/\sM\.\s/g, '\nM. ');
  return text
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean)
    ...
    .map((item) => ({
      ...
      sourceChain: [
        { label: 'Service-Public', kind: 'official' },
        { label: 'JORF', url: compositionUrl, kind: 'official', updatedAt: typeof record.date_modification_datetime === 'string' ? record.date_modification_datetime : undefined },
      ],
      ...OFFICIAL_SOURCE_STATUS,
    }));
}

export async function handleMinistersRequest(req, res) {
  const requestUrl = req.url ?? '';
  if (!requestUrl.startsWith('/api/ministers/')) return false;
  const url = new URL(requestUrl, 'http://localhost');

  if (requestUrl.startsWith('/api/ministers/profile-meta')) {
  ...
  if (requestUrl.startsWith('/api/ministers/wikidata-search')) {
    ...
    try {
      ...
      const data = await r.json();
      const POLITICAL_KEYWORDS = ['politi', 'ministre', 'premier', 'etat', 'état', 'fonctionnaire', 'depute', 'deputy', 'député', 'senator', 'sénateur', 'président', 'president', 'ambassad', 'secretaire', 'secrétaire', 'maire', 'élu', 'elu', 'personnalite', 'governor', 'officier', 'homme d', 'femme d', 'homme de', 'femme de'];
      const result = (data.search ?? []).find((item) => {
        const description = typeof item.description === 'string' ? item.description.toLowerCase() : '';
        return POLITICAL_KEYWORDS.some((keyword) => description.includes(keyword));
      }) ?? data.search?.[0];
  ...
  if (requestUrl.startsWith('/api/ministers/wikidata')) {
    ...
    try {
      ...
      const r = await fetch(apiUrl, { headers: { Accept: 'application/json' } });
      const data = await r.json();
      setJson(res, 200, data?.entities?.[id] ?? {});
    } catch {
      setJson(res, 502, {});
    }
    return true;
  ...
  if (requestUrl.startsWith('/api/ministers/prime-minister')) {
    try {
      const record = await fetchPrimeMinisterRecord();
      if (!record) {
        setJson(res, 404, {});
      ...
      const assignments = tryParseArray(record.affectation_personne);
      const pmAssignment = assignments.find((item) => item.fonction?.toLowerCase() === 'premier ministre') ?? assignments[0];
      const sites = tryParseArray(record.site_internet);
      const phones = tryParseArray(record.telephone);
      const references = pmAssignment?.personne?.texte_reference ?? [];
      const emails = pmAssignment?.personne?.adresse_courriel ?? [];
      const decreeReferences = tryParseArray(record.texte_reference);
      ...
      setJson(res, 200, {
        ...
        officeEmail: typeof record.adresse_courriel === 'string' && record.adresse_courriel ? record.adresse_courriel : emails[0]?.valeur,
        ...
        sourceChain: [
          { label: 'Service-Public', url: typeof record.url_service_public === 'string' ? record.url_service_public : undefined, kind: 'official' },
          { label: 'JORF', url: compositionReference?.valeur ?? references[0]?.valeur, kind: 'official', updatedAt: typeof record.date_modification_datetime === 'string' ? record.date_modification_datetime : undefined },
        ],
        ...OFFICIAL_SOURCE_STATUS,
      });
    } catch {
  ...
  if (requestUrl.startsWith('/api/ministers/composition')) {
    try {
      const composition = await fetchGovernmentCompositionFromJORF();
      setJson(res, 200, composition);
    } catch {
      setJson(res, 502, []);
    }
    return true;
  ...
  if (requestUrl.startsWith('/api/ministers/opendata')) {
    ...
    try {
      ...
      const datasets = (data.data ?? []).map((dataset) => {
        ...
        return {
          ...
          organization: typeof dataset.organization === 'object' && dataset.organization && 'name' in dataset.organization ? dataset.organization.name : undefined,
  ...
  if (requestUrl.startsWith('/api/ministers/agenda')) {
    const ministerId = url.searchParams.get('id') ?? '';
    const minister = GOUVERNEMENT.find((item) => item.id === ministerId);
    if (minister) {
      try {
        const officialItems = await fetchOfficialAgenda(minister);
        if (officialItems.length > 0) {
          setJson(res, 200, officialItems);
          return true;
        }
        const meta = await fetchOfficialProfileMeta({ prenom: minister.prenom, nom: minister.nom, siteMinistere: minister.siteMinistere, sourceUrl: minister.sourceUrl, isPresident: minister.isPresident });
        if (meta.agendaUrl) {
          setJson(res, 200, [{ title: `Consulter l'agenda officiel de ${minister.prenom} ${minister.nom}`, date: 'Agenda officiel', sourceLabel: meta.sourceLabel, url: meta.agendaUrl }]);
          return true;
        }
      } catch {}
    }
    ...
    try {
      ...
      const r = await fetch(minister.rssUrl, { headers: { Accept: 'application/rss+xml, text/xml', 'User-Agent': 'FranceMonitor/2.0' }, signal });
      const xml = await r.text();
      if (!r.ok || isBlockedHtml(xml)) {
        setJson(res, 200, []);
        return true;
      }
      const rssItems = [];
      ...
      setJson(res, 200, sortAgendaByDateDesc(rssItems).slice(0, 8).map(({ _isoDate, ...rest }) => rest));
    } catch {
      setJson(res, 200, []);
    }
    return true;
